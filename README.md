...
本アプリでは、利便性向上を目的として、検索操作の一部情報（検索した作品名・楽器・場面など）を収集することがあります。

- **収集目的**: アプリの利用状況把握・改善のため
- **収集内容**: 検索条件（作品名・楽器・場面等）、アクセス日時、使用ブラウザ情報（UserAgent）
- **個人情報**: 氏名・メールアドレス等の個人を特定できる情報は収集しません
- **保存場所**: 運営者のGoogleスプレッドシート（非公開）にのみ保存されます
- **第三者提供**: 収集した情報を第三者に提供することはありません
