# 管理者向けアクセス分析ダッシュボード 作業チェックリスト

## 作業の基準点

- 作業開始日: 2026-07-26
- 作業前ブランチ: `main`
- 作業前コミット: `8a89d6833fe9be8dc8b111faeea137b31ff387b3`
- 作業ブランチ: `codex/admin-dashboard-analytics`
- GAS・GA4・GitHub Pages・Sites は、各公開ゲートまで変更しない。
- `mahler-search-app/dic.html` と `mahler-search-app/data/` は生成物として扱い、直接の永続修正をしない。

### 作業前へ戻す基準

作業ブランチの変更が気に入らない場合は、`main` に戻れば作業前のコードへ戻れる。
公開後に問題が見つかった場合に備え、各公開前に対象コミット、GASデプロイID、Sitesバージョンを記録する。

---

## 0. 安全な作業環境

- [x] `gaswebapp-manual` の作業ツリーがクリーンであることを確認
- [x] `main` と `origin/main` が一致していることを確認
- [x] 作業前コミットを記録
- [x] 専用ブランチ `codex/admin-dashboard-analytics` を作成
- [ ] ダッシュボード側プロジェクトの現状を基準コミットまたは復元可能なスナップショットとして保存
- [ ] ダッシュボード側の全ファイルが未追跡である状態を解消
- [ ] 本作業で変更を許可するファイル一覧をフェーズごとに定義

## 1. 指標とJSON仕様の確定

- [ ] `daily.views` を「用語集 `dic.html` の閲覧数」とするか最終確認
- [ ] 実例クリック率を `click_view_example / dic.htmlの閲覧数` と定義
- [ ] `source_page` を「検索を実行したページ」と定義
- [ ] 検索ページへ移動する前のページは、移動イベント側の `source_page` として分離
- [ ] 検索総数を `view_search_results + search_no_results` と定義
- [ ] ページ名と実在パスの対応表を確定
- [ ] `translation` が見つからない検索語の返却方法を確定
- [ ] JSONの件数上限と並び順を確定

### 確認ゲート A

- [ ] 管理者が指標・用語・JSON仕様を確認
- [ ] 確認完了まで計測コードを変更しない

## 2. `sync-data.ps1` とGASデプロイの安全強化

- [ ] `.env` の既存GASデプロイIDだけを更新するようにする
- [ ] デプロイIDが見つからない場合は安全停止する
- [ ] 別のデプロイを自動選択するフォールバックを廃止
- [ ] 通常同期で新規デプロイを自動作成しない
- [ ] `manage_deploy.js` がデプロイ失敗を終了コードで返すようにする
- [ ] `update_env.js` が「最新版」ではなく固定デプロイIDを参照するようにする
- [ ] `sync-data.ps1` がGASデプロイ完了を確認してから次へ進むようにする
- [ ] `git add .` に依存せず、フェーズごとの許可リストでステージする方法を検討・実装
- [ ] 生成物がローカルソースコミットに入らないことを自動検査
- [ ] 想定外の変更があれば、コミット・GAS出力・pull前に安全停止
- [ ] 既存の通常同期を壊していないことを非公開状態で検証

### 確認ゲート B

- [ ] 変更差分と安全停止条件を管理者が確認
- [ ] 確認完了まで `sync-data.ps1` を実運用モードで実行しない

## 3. GA4イベント計測

- [ ] 現在の `view_search_results` / `search_no_results` の送信箇所をテストで固定
- [ ] 検索イベントへ `source_page` を追加
- [ ] `search_page_move` イベントを追加
- [ ] `search_page_move` に `source_page` を追加
- [ ] `search_page_move` に `destination_page` を追加
- [ ] `?q=` がある移動では `search_term` を追加
- [ ] リンク種別を区別する `link_type` を追加
- [ ] `click_view_example` の現行動作を維持
- [ ] 必要なら `click_view_example` へ `source_page` を追加
- [ ] 無効入力・中止・失敗検索ではイベントを送らない
- [ ] 0件検索では `search_no_results` を残す
- [ ] 静的ページとGASテンプレートの双方を整合させる
- [ ] 用語集の変更は `src/generate_dic_html.js` に入れる
- [ ] イベント送信失敗が検索・リンク移動を妨げないことを確認

### 確認ゲート C

- [ ] コード差分とイベント一覧を管理者が確認
- [ ] 確認完了までGAS・GitHub Pagesへ公開しない

## 4. GA4管理画面の手動設定

- [ ] `search_type` カスタムディメンションの有無を確認
- [ ] `composer` カスタムディメンションの有無を確認
- [ ] `result_count` カスタム指標の有無を確認
- [ ] `source_page` をイベントスコープのカスタムディメンションとして追加
- [ ] `destination_page` をイベントスコープのカスタムディメンションとして追加
- [ ] 必要なら `term` をイベントスコープのカスタムディメンションとして追加
- [ ] 拡張計測の「サイト内検索」が無効であることを確認
- [ ] データ保持期間を確認
- [ ] GAS実行アカウントが対象GA4プロパティを閲覧できることを確認
- [ ] GA4プロパティIDを確認

## 5. GAS Analytics Data API

- [ ] Analytics Data 高度なサービスをマニフェストへ追加
- [ ] 必要な読み取り専用OAuthスコープを追加
- [ ] Script Properties の `GA4_PROPERTY_ID` を利用する
- [ ] `period=7/30/90/180` だけを許可
- [ ] ダッシュボードAPI用GETルートを追加
- [ ] 日別の検索数・閲覧数・実例クリック数を集計
- [ ] ページ別の閲覧数・検索数・実例クリック数・検索ページ移動数を集計
- [ ] 検索語と検索元ページの組み合わせを集計
- [ ] 用語別検索回数とページ内訳を集計
- [ ] `Notes` シートから訳語を補完
- [ ] 個人情報・UserAgent・検索履歴の詳細本文をJSONへ含めない
- [ ] 0件検索語一覧をJSONへ含めない
- [ ] `CacheService` で短時間キャッシュ
- [ ] エラー時のJSON形式を固定
- [ ] APIを公開集計値だけに限定

## 6. GAS APIのローカル検証

- [ ] Data APIリクエストの構文を確認
- [ ] 各ディメンション・メトリクスの互換性を確認
- [ ] ダミー応答またはテスト関数でJSON変換を確認
- [ ] 7日応答を検証
- [ ] 30日応答を検証
- [ ] 90日応答を検証
- [ ] 180日応答を検証
- [ ] データがない日を0件として補完
- [ ] 日本時間の日付境界を確認
- [ ] JSONの値が有限の数値であることを確認
- [ ] `git diff --check` と構文検査を実施

### 確認ゲート D

- [ ] GAS・API差分とテスト結果を管理者が確認
- [ ] 確認完了までGASをデプロイしない

## 7. GAS・GitHub Pagesの限定公開作業

- [ ] 公開直前に `git status --short` を再確認
- [ ] 変更ファイルが許可リストと一致することを確認
- [ ] 公開前コミットを記録
- [ ] `sync-data.ps1` でGASソースを更新
- [ ] 固定デプロイIDが維持されたことを確認
- [ ] GAS Webアプリの更新を確認
- [ ] `exportAllDataToJson` の成功を確認
- [ ] 生成物がGASからGitHubへ更新されたことを確認
- [ ] `pull --rebase` 後の履歴と作業ツリーを確認
- [ ] GitHub Pages公開状態を確認
- [ ] 未確認の項目を完了扱いにしない

## 8. GA4受信検証

- [ ] オプトアウト拡張機能の影響を受けない環境を使用
- [ ] `view_search_results` をRealtimeで確認
- [ ] `search_no_results` をRealtimeで確認
- [ ] `click_view_example` をRealtimeで確認
- [ ] `search_page_move` をRealtimeで確認
- [ ] `source_page` と `destination_page` の値を確認
- [ ] 二重計測がないことを確認
- [ ] 24～48時間後に通常レポート／Data APIで確認

## 9. `sync-data.ps1` のダッシュボードAPI検査

- [ ] 同期後に固定GAS API URLを使用
- [ ] 7日JSONを自動検査
- [ ] 30日JSONを自動検査
- [ ] 90日JSONを自動検査
- [ ] 180日JSONを自動検査
- [ ] 必須キーと配列を検査
- [ ] HTMLや認証画面が返った場合は失敗扱い
- [ ] API検査失敗とGitHub Pages更新結果を分けて表示
- [ ] API検査失敗時に誤って全工程成功と表示しない

## 10. ダッシュボード側の実データ接続

- [ ] ダッシュボード側の基準コミットから専用ブランチを作成
- [ ] `GAS_API_URL` を固定GAS APIへ設定
- [ ] JSONのTypeScript型を確定
- [ ] 応答値を実行時検証
- [ ] ダミーパスを実在するパスへ修正
- [ ] 読み込み中・失敗・空データ表示を確認
- [ ] 実例クリック率の分母を確認
- [ ] 7・30・90・180日の切替を確認
- [ ] 棒グラフの表示を確認
- [ ] PC幅とスマートフォン幅で確認
- [ ] ローカルビルドとテストを実施

### 確認ゲート E

- [ ] 実データを使った安全なプレビューを管理者が確認
- [ ] 確認完了までSitesの新バージョンを公開しない

## 11. ダッシュボード公開

- [ ] 公開前のダッシュボードコミットを記録
- [ ] 正確なソース状態からSitesバージョンを保存
- [ ] Sitesバージョン番号を記録
- [ ] 承認されたバージョンだけを公開
- [ ] 本番URLのアクセス制御を確認
- [ ] 本番URLで実データを確認
- [ ] GAS API URLや秘密情報が不適切に公開されていないことを確認

## 12. 最終運用確認

- [ ] 通常のスプレッドシート更新を一件用意
- [ ] `sync-data.ps1` を一度実行
- [ ] 用語サイトの生成データ更新を確認
- [ ] GASコード・固定URLの維持を確認
- [ ] ダッシュボードAPI検査の成功を確認
- [ ] ダッシュボード再公開なしで最新値が表示されることを確認
- [ ] 作業ツリーがクリーンであることを確認
- [ ] 復元手順と運用手順をマニュアルへ記載

---

## 作業記録

各フェーズ完了時に、次を追記する。

- 日時
- 完了したチェック項目
- 変更ファイル
- コミット
- 実施した検証
- 公開の有無
- 未確認事項
- 復元先
