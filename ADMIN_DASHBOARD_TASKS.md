...
- [ ] ダッシュボード側プロジェクトの現状を基準コミットまたは復元可能なスナップショットとして保存
- [ ] ダッシュボード側の全ファイルが未追跡である状態を解消
- [ ] 本作業で変更を許可するファイル一覧をフェーズごとに定義
...
- [ ] ダッシュボード側の基準コミットから専用ブランチを作成
- [ ] `GAS_API_URL` を固定GAS APIへ設定
- [ ] JSONのTypeScript型を確定
- [ ] 応答値を実行時検証
- [ ] ダミーパスを実在するパスへ修正
- [ ] 読み込み中・失敗・空データ表示を確認
...
- [ ] 実データを使った安全なプレビューを管理者が確認
- [ ] 確認完了までSitesの新バージョンを公開しない
