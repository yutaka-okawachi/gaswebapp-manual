...
function trackSearchEvent(composer, searchType, params) {
    ...
    let resultCount = Number.isFinite(params && params.result_count)
        ? params.result_count
        : null;
    if (resultCount === null) {
        const preferredSelectors = ['.result-entry', '.search-result-item', '.result-a'];
        for (const selector of preferredSelectors) {
            const count = resultsDiv.querySelectorAll(selector).length;
            if (count > 0) {
                resultCount = count;
                break;
            }
    ...
    if (resultCount === null) {
        const countMatch = resultsDiv.textContent.match(/(\d+)件(?:見つかりました|ありました|該当しました)/);
        if (countMatch) resultCount = Number(countMatch[1]);
    }
    if (resultCount === null && /該当するデータが見つかりませんでした/.test(resultsDiv.textContent)) {
        resultCount = 0;
    }
    if (resultCount === null) return;

    const method = params && params.work_search_method;
    const payload = Object.assign({}, params || {}, {
        composer: composer,
        search_type: [composer.toLowerCase(), searchType, method].filter(Boolean).join('_'),
        result_count: resultCount
    });
    delete payload.work_search_method;
