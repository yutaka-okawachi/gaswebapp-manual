...
通常の `02_RUN_SYNC.bat` を実行すると、公開前にブラウザーで確認画面が開きます。検索や用語集を確認してから、画面上部の「確認しました。この内容を公開する」を押してください。押すまではGitHubへ送信しません。画面確認中に中断しても、同じバッチで再開できます。

GASのプログラム更新が必要なときは、GAS更新前にも現在のデータで確認画面が開きます。その後に最新のデータを取得し、公開直前にもう一度確認できます。詳細・確認だけのコマンドは開発者ガイドの第2章を参照してください。
