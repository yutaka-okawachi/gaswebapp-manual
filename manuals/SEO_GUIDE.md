...
| rs_synopsis.html | RSあらすじ集（Straussオペラ作品の概要一覧） |
| rw_synopsis.html | RWあらすじ集（Wagner楽劇・オペラ作品の概要一覧） |
...
3. **`sync-data.ps1` への登録**
   サイトマップの `<lastmod>`（最終更新日）を自動更新させるため、[sync-data.ps1](../sync-data.ps1) 内の `$pathMappings` 変数（ハッシュテーブル）に、以下のようにファイルパスのマッピングを登録してください。
   ```powershell
   'mahler-search-app/新ファイル名.html' = 'mahler-search-app/新ファイル名.html'
   ```
