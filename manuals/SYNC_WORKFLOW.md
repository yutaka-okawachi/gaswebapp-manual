...
5.  **新しいデプロイの作成 (必要時)**: コードが更新された場合、自動的に新しいバージョンをデプロイし、古いデプロイを削除します。
6.  **フロントエンド設定の同期**: 新しいデプロイの URL を取得し、`mahler-search-app/js/app.js` 内の通知先 URL 設定を自動的に更新します。
7.  **データの同期**: GAS ウェブアプリのエンドポイント (`exportAllDataToJson`) を叩き、最新のデータを JSON としてダウンロードして `data/` ディレクトリに保存します。
