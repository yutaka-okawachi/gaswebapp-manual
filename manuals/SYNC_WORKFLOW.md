...
9.  **サイトマップの動的更新 (SEO最適化)**: GitHub から最新データを Pull した後、実際に更新された HTML ファイル（`dic.html` や追加されたあらすじ集 `rs_synopsis.html` など）を自動検知し、`sitemap.xml` の `<lastmod>` (最終更新日) を最適化してコミット・プッシュします。（※新規のHTMLファイルを追加した場合は、スクリプト内の `$pathMappings` に追加登録が必要です）
