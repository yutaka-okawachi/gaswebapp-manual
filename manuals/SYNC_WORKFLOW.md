...
- スプレッドシートのデータだけを直した場合: まず公開サイトで反映状況を確認し、必要に応じて `.\sync-data.ps1` を実行します。
- `src/` や `mahler-search-app/` のファイルを直した場合: `clasp login` が正しい Google アカウントで済んでいることを確認してから `.\sync-data.ps1` を実行します。
