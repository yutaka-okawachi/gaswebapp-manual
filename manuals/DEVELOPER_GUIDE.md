...
通常は従来どおり `02_RUN_SYNC.bat` または `.\sync-data.ps1` の一回実行で完了します。公開前にブラウザーで目視確認するため停止し、画面の確認ボタンを押すと後続処理に進みます。
...
3. GitHub認証と公開設定を確認し、ソースをローカルにコミットします。リモートに未取得の変更がある場合は、本番変更前に停止します。
4. GASコードの更新が必要な場合、現在のデータによるプレビューを開きます。「GAS更新と最新データの取得に進む」を押した後、固定デプロイを更新し、埋め込まれたソースハッシュを確認します。同じコードであればGASの版を追加しません。
...
7. 最新データを含む公開前プレビューが開きます。検索・辞書リンク・画面を確認し、「確認しました。この内容を公開する」を押します。プレビューでは計測と通知を抑止します。
...
`.sync-state/` はこのPC専用の再開記録です。認証トークンを保存しません。公開待ち・検証失敗で停止した場合、同じコマンドを再実行してください。公開ボタンを押す前に編集した場合は再構築・再確認します。承認済みの公開確認を再開する場合は再承認しません。プレビュー中に編集された内容は公開しません。
