...
検索数・閲覧数・実例クリック数・訪問経路・継続利用はGA4 Data APIの集計です。スプレッドシートの「検索履歴」タブを編集しても、これらの利用状況集計は直接変更されません。「Notes」タブは検索語の訳語補完に利用され、変更内容は `mahler-search-app/data/dic_notes.json` と生成済み `mahler-search-app/dic.html` に反映されます。`dic.html` と `data/` は生成物なので、永続的な修正はスプレッドシートまたはGAS側の生成元へ入れてください。
