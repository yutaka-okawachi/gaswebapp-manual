...
辞書候補の観測・判定・採用確認は，本番の `Notes` と `語形対応` から分離して行います．実験用タブへ追加されただけでは公開サイトは変わりません．

`Notes_正規化テスト` の見出しは，明示的に統合した項目を除き，本番用 `Notes` の見出しを保持します．例えば `Abstufungen` を `Abstufung` に置き換えず，`辞書語形・検索対応_実験` のA列を `Abstufungen`，B列を関連語形 `Abstufung` とします．B列の語を用語検索したときはA列の見出しへ移動し，「実例を見る」では両方の語形を検索対象に含めます．関連語形にも独立見出しがある場合は，その独立見出しを優先します．

本番用 `Notes` のA～C列は `Notes_正規化テスト` と一致させています．D列以降は変更していません．本番用 `語形対応` はA～F列を `見出し`，`関連語形`，`語形区分`，`用語検索`，`実例検索`，`補足` とし，`辞書語形・検索対応_実験` の採用内容を反映しています．`sync-data` は本番用 `Notes` と `語形対応` を読み，実験用タブは読みません．
...
| タブ | 役割 | 通常検索から自動更新するか |
| --- | --- | --- |
| `検索結果未登録語_正規化テスト` | 用語検索で1件以上ヒットしたが，検索語そのものが辞書見出し・登録済み関連語形にない場合の観測先 | 公開採用後は更新する |
| `Jev判定_正規化テスト` | Jevの分類助言とM列 `人による確認` による採否 | 更新しない |
| `辞書語形・検索対応_実験` | 人が採用した関連語形と検索範囲 | 更新しない．明示操作で反映する |
| `未登録語候補_正規化テスト` | 人が採用した新規用語候補とGPT草案 | 更新しない．明示操作で反映する |
...
1. 通常利用者がGM・RS・RWの「用語から検索」を実行します．検索結果が1件以上あり，検索語が辞書見出し・登録済み関連語形にない場合だけ，`検索結果未登録語_正規化テスト` へ同一正規化語形ごとに集約します．0件検索とローカルプレビューでは記録しません．管理者モードでは既定で記録せず，5-2節の明示的な候補観測ON時だけ記録します．
2. 必要な候補だけをJevで判定し，結果を `Jev判定_正規化テスト` へ保存します．Jevの実行は通常検索・通常同期とは別の明示操作です．
3. 人が分類・対応見出し・確信度を確認し，M列を `採用`，`却下`，`保留` のいずれかにします．
4. スプレッドシートの `📚 辞書候補` → `採用済み判定を実験用タブへ反映` を実行します．`INFLECTION`，`ORTHOGRAPHIC_VARIANT`，`TYPO` の採用行は `辞書語形・検索対応_実験` へ，`NEW_TERM_CANDIDATE` の採用行は `未登録語候補_正規化テスト` へ追加または更新します．
5. `辞書語形・検索対応_実験` ではA列とB列の組合せを重複追加しません．`INFLECTION` は自動処理だけで格変化形・複数形・活用形等を断定せず，C列を `語形変化（要分類）` として人の確認を残します．
6. 本番採用時は，実験用タブ全体を確認してから本番用 `Notes`・`語形対応` へ移行し，通常の `sync-data.ps1` を実行します．

`Jev判定_正規化テスト` と `検索結果未登録語_正規化テスト` は上記の経路で使用します．`未解決語観測_正規化テスト` は削除・転用せず，0件検索の履歴を再評価するときの入力元として残します．旧履歴を将来取り込む場合も，新方式の1件以上ヒットした観測と混在させず，Jev判定前に由来を明示して人が選別します．

`Notes_正規化テスト` と `辞書語形・検索対応_実験` を本番データへ反映せずに画面確認する場合は，対象範囲を取得したローカル一時ファイルを用意してから `node scripts/preview-experimental-dictionary.js` を実行します．プレビュー上部に実験サイトであることが表示され，検索通知・候補観測は送信されません．このプレビューは `sync-data`，commit，push，deployを行いません．
...
辞書整備中に限り，管理者モードのまま `検索結果未登録語_正規化テスト` へ候補を記録したい場合は，同じブラウザの作業タブで次を開きます．
