...
GM・RW・RSの「用語から検索」には，辞書の「実例を見る」から移動した場合，実際に追加検索する登録語形・別綴りを具体的に列挙します．例えば `stürzt` に `stürzen` が登録されている場合は，「stürzen」も見出し「stürzt」に登録された語形変化として検索対象であることを表示します．検索欄を編集したときはこの案内を隠し，通常検索に戻します．検索欄から直接検索した場合は，関連語形の案内を表示せず，入力語に一致する実例だけを検索します．対応表にない語形は自動推定しません．
...
6. `語形対応` ではA列とB列の組合せを重複追加しません．関連語形を追加した時点で，その語形をRS・RW・GMから調査し，対応する見出しのNotes C列へ `[RS: Oper]`，`[RW: Oper]`，`[GM]` を既存記載と重複しないよう追記します．既存の対応をまとめて再確認する場合は，スプレッドシートの `📚 辞書候補` → `既存語形対応の出典を調査` を実行します．`INFLECTION` は自動処理だけで格変化形・複数形・活用形等を断定せず，C列を `語形変化（要分類）` として人の確認を残します．
7. 新規用語は草案・B列本文・出典を確認してから `Notes` に見出し順で登録します．公開サイトに反映する時点で通常の `sync-data.ps1` を実行します．

#### 語形対応の一括調査を確認する

`語形対応_一括調査_` で始まり所定のA～U列見出しを持つ調査タブを開き，`📚 辞書候補` → `一括調査の語形対応を確認` を選びます．複数の調査タブがある場合はダイアログで切り替えられます．表示対象は，RS・RW・GMの実例が1行以上あり，Jev関係に `[INFLECTION]` を含み，確信度が90%以上で，人の採否が空欄の行です．新しいJev判定やAPI課金は発生しません．

見出し，関連語形，実例位置・文脈，Jevの文法区分を1件ずつ確認します．語形区分と任意の補足を入力して「この語形を採用して登録」を押すと，`語形対応` のA～F列に追加し，該当するNotesのC列出典を調査してから，一括調査タブの該当行を削除します．同じ見出し・関連語形の組合せは重複追加しません．関連語形が独立したNotes見出しでもある場合，用語検索は `対象外`，実例検索は `対象` です．「保留」「却下」は一括調査タブのR・S列に記録して次の候補へ進み，行は残します．登録済みの組合せが表示された場合は，登録先を変更せず，確認して調査行だけを削除できます．

採用時に途中でエラーが出た場合は `語形対応` と調査タブの双方を確認してください．登録先には追加済みで調査行が残っている場合，画面を開き直すと登録済みとして表示され，その行だけを整理できます．シートの更新を公開サイトに反映するには通常の `sync-data.ps1` が必要です．

登録処理はほかのGAS処理と共有するロックで保護します．実例の走査と次候補の読込みはロックの外で行います．ロックを30秒以内に取得できなかった場合は，未書込みであることをサーバーが確認して画面へ返し，画面は待機を表示して最大2回再試行します．通信エラーや書込み後の例外は自動再送しません．
...
データ取得時にローカルソースと異なる `sourceHash` のスナップショットが返った場合も、同じ実行IDで再取得します。ハッシュが一致した結果だけを検証・配置し、再試行後も一致しなければ公開せず停止します。
