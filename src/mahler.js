...
function doGet(e) {
  ...
  const name = (page === 'richard_strauss') ? 'richard_strauss'
    : (page === 'richard_wagner') ? 'richard_wagner'
      : (page === 'terms_search') ? 'terms_search'
        : (page === 'rs_terms_search') ? 'rs_terms_search'
          : (page === 'rw_terms_search') ? 'rw_terms_search'
            : (page === 'list') ? 'list'
              : (page === 'notes') ? 'notes'
                : 'index';

  const template = HtmlService.createTemplateFromFile(name);
  if (name === 'richard_strauss' || name === 'richard_wagner') {
    const sheetName = (name === 'richard_strauss') ? 'RS幕構成' : 'RW幕構成';
    template.sceneOptions = getSceneOptions(sheetName);
  }
...
function getEnglishTerms() {
  ...
  return data.flat().filter(term => term);
}
...
function searchByTerm(query) {
  ...
  results.forEach(row => {
    ...
    segments.forEach(segment => {
      ...
      const instruments = instrCodes.map(code => {
        if (dReverseMap[code] != null) return dReverseMap[code];
        return code;
      }).join(', ');
...
function searchData(choice1Arr, choice2Arr, includeOrchestraAll) {
  ...
  return totalMatches === 0 ? '該当するデータがありません。' : `${totalMatches}件ありました。\n\n${resultHTML}`;
}
...
function sendSearchNotification(subject, body) {
  ...
  try {
    ...
    Logger.log(`検索通知メールを ${recipient} に送信しました。`);
  } catch (error) {
    Logger.log(`検索通知メールの送信に失敗しました: ${error}`);
  }
}
...
function formatMovementNumber(a, b) {
  const specialMapping = {
    c: {
      c1: "Wenn mein Schatz Hochzeit macht", c2: "Ging heut' morgen über's Feld",
      c3: "Ich hab' ein glühend Messer", c4: "Die zwei blauen Augen von meinem Schatz"
    },
    d: {
      d01: "Der Schildwache Nachlied", d02: "Verlorne Müh'!", d03: "Trost im Unglück",
      d04: "Das himmlische Leben", d05: "Wer hat dies Liedel erdacht?", d06: "Das irdische Leben",
      d07: "Urlicht", d08: "Des Antonius von Padua Fischpredigt", d09: "Rheinlegendchen",
      d10: "Lob des hohen Verstands", d11: "Lied des Verfolgten im Turm",
      d12: "Wo die schönen Trompeten blasen", d13: "Revelge", d14: "Der Tamboursg'sell"
    },
    e: {
      e1: "Nun will die Sonn' so hell aufgeh'n", e2: "Nun seh' ich wohl, warum so dunkle Flammen",
      e3: "Wenn dein Mütterlein", e4: "Oft denk' ich, sie sind nur ausgegangen",
      e5: "In diesem Wetter, in diesem Braus"
    },
    f: {
      f1: "Blicke mir nicht in die Lieder!", f2: "Ich atmet' einen linden Duft",
      f3: "Ich bin der Welt abhanden gekommen", f4: "Um Mitternacht", f5: "Liebst du um Schönheit"
    }
  };
...
/**
 * チャンキング（分割）を利用して大きなデータをキャッシュに保存する関数
 * @param {string} key - ベースとなるキャッシュキー
 * @param {Object} data - 保存するデータ（JSONオブジェクト）
 * @param {number} expirationInSeconds - キャッシュの有効期限（秒）
 */
function setChunkedCache(key, data, expirationInSeconds) {
  ...
  // 90KBごとにデータを分割 (上限100KBより余裕を持たせる)
  ...
  // チャンクの数と、各チャンクデータをオブジェクトにまとめる
  const cacheData = {
    [`${key}_count`]: chunks.length // チャンクの総数を保存
  };
  chunks.forEach((chunk, index) => {
    cacheData[`${key}_${index}`] = chunk;
  });

  // putAllで複数のデータを一度にキャッシュに保存
  cache.putAll(cacheData, expirationInSeconds);
  Logger.log(`${key} を ${chunks.length} 個のチャンクに分割してキャッシュしました。`);
}

/**
 * 分割して保存されたデータをキャッシュから読み込み、結合して返す関数
 * @param {string} key - ベースとなるキャッシュキー
 * @returns {Object|null} 復元されたデータ、またはキャッシュが存在しない場合はnull
 */
function getChunkedCache(key) {
  ...
  // getAllで全チャンクを一度に取得
  const cacheResult = cache.getAll(keys);

  // 取得したチャンクを結合
  let jsonString = '';
  for (let i = 0; i < count; i++) {
    const chunk = cacheResult[`${key}_${i}`];
    if (!chunk) {
      // 途中のチャンクが欠けていたら失敗とみなし、nullを返す
      Logger.log(`キャッシュチャンク ${key}_${i} が見つかりませんでした。`);
      return null;
    }
    jsonString += chunk;
  }

  // 結合したJSON文字列をオブジェクトに変換して返す
  Logger.log(`${key} を ${count} 個のチャンクから復元しました。`);
